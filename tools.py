...
class ScheduleMeetingRequest(BaseModel):
    ...
    time: str = Field(description="Start time of the meeting in HH:MM (24-hour) or HH:MM AM/PM format")
    duration_minutes: int = Field(default=30, description="Duration in minutes (e.g. 15, 30, 45, 60)")
    attendees: List[str] = Field(description="List of attendee email addresses or names")
    notes: Optional[str] = Field(default="", description="Optional agenda or notes for the meeting")


class CancelMeetingRequest(BaseModel):
    meeting_id: str = Field(description="Unique ID of the meeting to cancel (e.g., MTG-101)")
...
def get_available_slots(date: str) -> dict:
    ...
    """
    # Simulated availability schedule
